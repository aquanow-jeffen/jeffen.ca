export const resumeData = [
  {
    name: "Aquanow",
    time: "Jun 2021 - Present",
    title: "Full-Stack Engineer, Technical Product Owner",
    keywords: "",
    description: (
      <>
        <div>
          Closely work with CTO on AWS Full-stack application design, dev
          process and tool standardization across-team. Manage one of Aquanow
          product lines.
        </div>
        <details>
          <div>
            • Transform AWS Lambda Javascript codebase to typescript codebase
            with Linting, Code Coverage, Webpack and CI/CD pipeline enforced
            structure.
          </div>
          <div>
            • Transform CRA/ANTD/Redux based SPA to NextJS/MUI/Redux-toolkit
            with uniformed structure
          </div>
          <div>
            • Full stack typescript development, testing, performance and
            monitoring
          </div>
          <div>
            • Conduct Scrum ceremonies(Agile), manage full product development
            cycle from planning to documentation and deliver features based on
            roadmap.
          </div>
          <div>• Provide client meeting, onboarding and technical support</div>
        </details>
      </>
    ),
  },
  {
    name: "Central 1 Credit Union",
    time: "Jun 2019 - May 2021",
    title: "Web Developer 3",
    description: (
      <>
        <div>
          Maintain a requireJS based micro-FE component library and framework.
          Focused on robustness and a11y. Developed several internal developer
          tools/websites to improve cross-team productivity.
        </div>
        <details>
          <div>
            • Build, maintain and improve performance on enterprise UI-component
            library(micro-FE) for client using Typescript, AngularJS, Vanilla
            JS, Sass, webpack.
          </div>
          <div>
            • Research, maintaining and provide insights on 3rd-party
            Platform(Backbase.com) source code built with Javascript
          </div>
          <div>• Write unit testing using Karma, Jasmine</div>
          <div>
            • Develop and automate Visual testing and E2E testing using
            backstopJS & Cypress. Visualize testing report and integrate with
            CI/CD pipeline.
          </div>
          <div>
            • Design, develop and deliver a browser extension to help visualize
            and manage micro-frontend components on a production page. Implement
            request interceptor, communication and 3rd-party integration(Jira,
            deployment information).
          </div>
          <div>
            • Design, maintain and improve performance on SCSS theming system
            for 70+ clients.
          </div>
          <div>
            • Build and maintain nodeJS CLI tools(yeoman generator, theme CI/CD
            builder)
          </div>
          <div>
            • Follow Agile development practice using Jira, Kanban & BitBucket
            to manage software development cycle
          </div>
        </details>
      </>
    ),
  },
  {
    name: "OTT Financial Group",
    time: "Aug 2017 - May 2019",
    title: "Senior Frontend Developer",
    keywords: "",
    description: (
      <div>
        <div>
          Lead frontend team on several products from planning to delivery.
          Focused on SPA, form, performance, and platform usability
        </div>
        <details>
          <div>
            • Research and uniform frontend components, process and
            infrastructure across products
          </div>
          <div>
            • Designed a web component that powers online payments integration (
            <a
              href="https://docs.ottpay.com/paybutton"
              target="_blank"
              rel="noreferrer"
            >
              https://docs.ottpay.com/paybutton
            </a>
            ). Lead from design to delivery of the product.
          </div>
          <div>
            • Lead development of several internal product frontend from ground
            up. Including: Online Ticket Application, Merchant dashboard, CRM
            system with Angular and ReactJS
          </div>
          <div>
            • PWA and Hybrid App with Ionic: QA Code scanner, analytic
            dashboard, online form, etc.
          </div>
          <div>
            • Existing system performance monitoring and site statistics using
            Google Analytics.
          </div>
          <div>
            • Leadership tasks: Interview, mentor new team members to improve
            their skills and knowledge. Maintain documentation and knowledge
            base.
          </div>
        </details>
      </div>
    ),
  },
  {
    name: "Freelancer",
    time: "May 2016 - Aug 2017",
    title: "Javascript developer",
    keywords: "",
    description: (
      <div>
        Sole developer on a House rental website of location based searching,
        uploading rental information and images, 3rd party service integration
        <details>
          <div>
            • Design and communicate with client to finalize development plan
          </div>
          <div>
            • Page styles with SemanticUI and state management with RXJS and
            NGRX on Angular
          </div>
          <div>• Design and develop mongoDB schema</div>
          <div>
            • API and Express security based on helmet and JsonWebToken(JWT)
          </div>
          <div>
            • Developed housing view and search service based on Google Map API
            from front-end to back-end
          </div>
          <div>
            • Deployed on DigitalOcean using Nginx with reverse proxy and SSL A+
            rating
          </div>
        </details>
      </div>
    ),
  },
  {
    name: "University of Ottawa, Canada",
    time: "2015 - 2017",
    title: "Master of Engineering",
    description: "",
  },
  {
    name: "Seoul National University, South Korea",
    time: "2014 - 2015",
    title: "Korean Language and Culture",
    description: "",
  },
  {
    name: "Beijing University of Chemical Technology, China",
    time: "2010 - 2014",
    title: "Bachelor of Engineering",
    description: "",
  },
];
